...
public final class Constants {
     ...
     *
     * <p>
     * <b>CHANGE BEFORE EACH MATCH</b> — label each physical battery with a
     ...
     *
     * <p>
     * Default {@code 0} means "unset" — treat those logs with caution.
     */
    public static final int BATTERY_ID = Match.BATTERY_ID;

    // ==================== Operating Mode ====================

    /**
     * Robot operating mode. Drives {@link com.adambots.Robot}'s AdvantageKit
     * receiver
     * configuration:
     * <ul>
     * <li>{@link Mode#REAL} — running on a real roboRIO. WPILOGWriter +
     * NT4Publisher active.</li>
     * <li>{@link Mode#SIM} — desktop simulation. NT4Publisher only (no disk).</li>
     * <li>{@link Mode#REPLAY} — re-run a recorded log through the code for
     * debugging.
    ...
    /**
     * Set per deploy target. REAL for robot, SIM for desktop, REPLAY for log
     * replay.
     */
     ...
     * <ul>
     * <li>{@link com.adambots.logging.LogUtil.Level#ESSENTIAL} — quals/comp. ~25
     * signals.</li>
     * <li>{@link com.adambots.logging.LogUtil.Level#DIAGNOSTIC} — practice /
     * post-match tuning.
     * Adds vision pipeline internals, per-module swerve, command durations.</li>
     * <li>{@link com.adambots.logging.LogUtil.Level#DEBUG} — bench only. Adds
     * high-rate
     * tuning data (lead-comp math, PID internals).</li>
     * </ul>
     * A change requires {@code ./gradlew clean deploy} to take effect (static final
     * inline).
     */
     ...
     *             ESSENTIAL.
     *             Remove after all call sites migrate to
     *             {@link com.adambots.logging.LogUtil}.
     */
    @Deprecated
    public static final boolean CURRENT_LOGGING = true;

    /**
     * @deprecated Replaced by {@link #LOG_LEVEL} — set to {@code DIAGNOSTIC} or
     *             higher for
     *             the same effect. Remove after all call sites migrate to
     *             {@link com.adambots.logging.LogUtil}.
     */
    @Deprecated
     ...
     * Speed limits, gearing, and dimensions are configured in YAGSL JSON and
    ...
    public static final class ShooterConstants {
        ...
        public static final double kFlywheelToleranceRPS = 3.5; // was 2.5 — shot boost adds 3 RPS above base target,
                                                                // causing at_speed chatter at 2.5
        ...
        public static final double kFlywheelSupplyCurrentLimit = 25.0; // was 40 — power-budget calc peak 13.7A @ 4000
                                                                       // RPM, back-EMF margin to 25A;
                                                                       // battery-friendlier during shooter+drive
                                                                       // simultaneity
        ...
        /**
         * Extra RPS added during feed to compensate for ball energy transfer to
         * flywheel.
         */
    ...
    public static final class TurretConstants {
        ...
        // kP: 20 × 0.031 rot/deg = 0.62V per degree error — enough to correct without
        // overshoot
        ...
        public static final double kTurretKS = 0.10; // static friction compensation (Volts) — planetary reduces
                                                     // stiction
        ...
        public static final double kTurretMotorGearRatio = (200.0 / 20.0) * 4.0; // motor → turret (20T pinion + 4:1
                                                                                 // planetary)
        ...
        /**
         * Pot reading (degrees) when turret is at 0° — determine empirically via
         * dashboard
         */
        public static final double kTurretPotAtZeroDeg = 3046.0;
        /**
         * Pot reading (degrees) when turret is at max — determine empirically via
         * dashboard
         */
         ...
         *
         * <p>
         * Derivation: the pot has its own 18T pinion (no planetary), so
         * pot travel / pot gear ratio = turret range.
         *
         * <p>
         * {@code Math.abs(...)} keeps the range positive regardless of which
         ...
         * (since 2026-04, after pot replacement).
         * {@code TurretSubsystem.getPotAngleDegrees()}
         * is already polarity-agnostic; only this range magnitude needed fixing.
         */
        public static final double kTurretMaxDegrees = Math.abs(kTurretPotAtMaxDeg - kTurretPotAtZeroDeg)
                / kTurretPotGearRatio;
        ...
        /**
         * Max percent output for joystick jog. Joystick input is squared for fine
         * control.
         */
    ...
    public static final class TurretTrackingConstants {
         ...
         * Turret leads the setpoint by robotAngVel × this value to compensate for
         * rotation.
         */
        public static final double kAngularVelLeadTime = 0.03;

        /**
         * Projectile time-of-flight (seconds) for <b>translational</b> lead
         * compensation.
         * Shifts the aim point by {@code -fieldVelocity × kShotLeadTimeSec} so the
         * turret
         * aims ahead of the hub to account for the robot's drift during the fuel's
         * flight.
         *
         * <p>
         * <b>Zero disables it</b> — the aim point reduces exactly to the static hub
         ...
         *
         * <p>
         * Tuning: start at 0.15–0.20s for FUEL at typical scoring ranges (3–4 m,
    ...
    // ==================== VisionConstants ====================
    /**
     * Constants for the vision system including camera names, positions, and pose
     * estimation parameters.
     *
     * <p>
     * Camera Position Coordinate System (robot-centric):
     * <ul>
     ...
     * <li>Pitch: Rotation around Y-axis (camera tilting up/down, negative = pitched
     * down)</li>
     * <li>Yaw: Rotation around Z-axis (camera rotated left/right, 0 = facing
     * forward)</li>
     * </ul>
     */
    public static final class VisionConstants {
        ...
        // Robot center = the point on the floor exactly between the four swerve
        // modules.
        //
         ...
         * Derived from geometric center of all hub tag positions in
        ...
        /**
         * Vision mode: 0 = Camera-only, 1 = Pose-only, 2 = Hybrid (camera primary, pose
         * fallback), 3 = Blended (weighted avg)
         */
        public static final int kVisionMode = 0;

        /**
         * Blend weight for mode 3: fraction of camera vs pose. 0.6 = 60% camera, 40%
         * pose.
         */
        public static final double kVisionBlendWeight = 0.6;

        /**
         * Max disagreement (degrees) between camera and pose before blend falls back to
         * camera-only.
         */
        public static final double kBlendDisagreementThreshold = 10.0;

        /**
         * Vision std dev scaling per m/s of robot speed. At 2 m/s: stdDevs *= 1 + 2*1.0
         * = 3x.
         */
         ...
         * 0.04 = 4% new, 96% prior (example: Team 6328). Higher = more responsive but
         * noisier.
         */
         ...
         * At 50, a full drain at -1/frame takes 1 second — survives long gaps between
         * detections.
         */
    ...
    public static final class IntakeConstants {
        ...
        // Reduced for lighter roller intake (was wheels) — tune from these starting
        // points.
        public static final double kArmP = 35; // Volts per rotation of error
        public static final double kArmI = 0.0; // Volts per rotation*second of error (almost never needed with proper
                                                // kG)
        public static final double kArmD = 0.08; // Volts per rotation/second of error
        public static final double kArmKV = 0.12; // Volts per rotation/second of velocity (motor characteristic,
                                                  // unchanged)
        ...
        public static final double kArmKG = 0.30; // Volts to hold arm horizontal — bumped 0.20→0.30 (2026-04-23) after
                                                  // practice logs showed motor stalling at 140° upward. Needs bench
                                                  // tuning for final value; our back-calc suggests ~1.0 V for actual
                                                  // arm mass.
        ...
        public static final int kRollerStatorCurrentLimit = 40; // 45→35 (Q-sag MICMP1) → 40 (Apr-25 logs showed 13%
                                                                // chronic pinning at 35; 40 still below original 45,
                                                                // supply gate at 30A)
    ...
    /**
     * Simulation constants — approximate values for code testing, not hardware
     * tuning.
     */
    public static final class SimConstants {
        ...
        // sensorToMechanismRatio and must stay 1.0 until position targets are
        // recalibrated).
